...
**Simplicity criterion**: All else being equal, simpler is better. A small improvement that adds ugly complexity is not worth it. Conversely, removing something and getting equal or better results is a great outcome — that's a simplification win. When evaluating whether to keep a change, weigh the complexity cost against the improvement magnitude. A 0.11 RMSE improvement that adds 20 lines of hacky code? Probably not worth it. A 1.0 RMSE improvement from deleting code? Definitely keep. An improvement of ~0 but much simpler code? Keep.
...
```
commit	rmse	memory_gb	status	description
```
...
5. Read out the results: `grep "^rmse:\|^peak_vram_mb:" run.log`
